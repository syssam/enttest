...
// The HashtagToPostFunc type is an adapter to allow the use of ordinary
// function as HashtagToPost mutator.
type HashtagToPostFunc func(context.Context, *ent.HashtagToPostMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HashtagToPostFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.HashtagToPostMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HashtagToPostMutation", m)
	}
	return f(ctx, mv)
}
