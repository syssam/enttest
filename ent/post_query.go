...
func (pq *PostQuery) QueryHashtagToPost() *HashtagToPostQuery {
	query := &HashtagToPostQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withHashtagToPost: pq.withHashtagToPost.Clone(),
...
func (pq *PostQuery) WithHashtagToPost(opts ...func(*HashtagToPostQuery)) *PostQuery {
	query := &HashtagToPostQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withHashtagToPost = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withHashtagToPost; query != nil {
		...
		query.Where(predicate.HashtagToPost(func(s *sql.Selector) {
			s.Where(sql.InValues(post.HashtagToPostColumn, fks...))
		}))
